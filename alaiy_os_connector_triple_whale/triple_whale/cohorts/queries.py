...
# Refunded value per acquisition channel per day.
#
# refunds_table carries source_name, but that is the sales channel (web, pos)
# rather than the marketing channel that won the order. pixel_orders_table
# holds the attribution, so refunds join through order_id to reach it.
#
...
SELECT
    ...
    sum(r.total_refunded_price)     AS refunded,
    sum(r.total_refunded_cogs)      AS refunded_cogs,
    countDistinct(r.refund_id)      AS refunds,
    countDistinct(r.order_id)       AS refunded_orders
FROM refunds_table r
INNER JOIN pixel_orders_table p ON p.order_id = r.order_id
WHERE r.event_date BETWEEN @startDate AND @endDate
  AND p.channel != ''
GROUP BY event_date, channel
ORDER BY event_date DESC, refunded DESC
"""
